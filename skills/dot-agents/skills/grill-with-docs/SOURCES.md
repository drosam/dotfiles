...
| Former local `grill-me/SKILL.md`, `SPEC.md` (absorbed into this skill) | canonical local | high | Interview steps, one-at-a-time pace, blocked-tool rule, explicit handoff |
...
- Kept `grill-with-docs` as the sole interview skill; removed `grill-me` and its live symlinks by user decision.
- Preserved independent question rounds, prerequisite ordering, codebase fact discovery, optional workers with direct-inspection fallback, pace/timebox preferences, stop conditions, and explicit implementation handoff.
- Broadened the description to cover plain "grill me" requests; retained manual invocation, inline glossary writes, and explicit ADR acceptance.
- Earlier validation below predates this consolidation; it is not a new runtime-validation result.
