...
- Before generating/reviewing code or editing work artifacts, inspect this folder explicitly. Select existing work only from an explicit user path, current-session confirmation, or validated reciprocal link from an already-selected document. Otherwise shortlist candidate paths with title/status/scope and ask with the question tool, even for one candidate. Offer Other / None—standalone work. Never infer selection from filename, matching slug, branch, recency, or unavailable prior-session memory. If unable to ask, report the blocker, not a guess. Ordinary explicit work with no artifacts and clear new standalone requests do not require document creation.
...
- Pass confirmed paths, Work-ID, status, constraints and authorized scope to workers, or confirmed no-artifact context. Workers missing identity must return candidate paths and a clarification request, never guess. Selection/design approval is not implementation authorization.
